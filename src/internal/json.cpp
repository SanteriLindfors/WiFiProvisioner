 ...
 */
#define END_CHARACTER_ENCOUNTERED(obj, index)                                  \
...
json::reader::push_result json::reader::push(const char next) {
  ...
  if (this->length() == 0 && std::isspace(next))
    return reader::ACCEPTED;

  // Get the type
  const json::jtype::jtype type =
      json::jtype::peek(this->length() > 0 ? this->front() : next);
...
#if DEBUG
  if (result == ACCEPTED)
    assert(this->length() - start_length == 1);
  else
    assert(this->length() == start_length);
#endif
...
bool json::reader::is_valid() const {
  ...
  case jtype::jbool:
    if (this->length() < 4)
      return false;
    if (this->length() == 4 && *this == "true")
      return true;
    if (this->length() == 5 && *this == "false")
      return true;
    return false;
  case jtype::jnull:
...
bool is_hex_digit(const char input) {
  return IS_DIGIT(input) || (input >= 'a' && input <= 'f') ||
         (input >= 'A' && input <= 'F');
}
...
json::reader::push_result json::reader::push_array(const char next) {
  ...
  begin_reading_value:
    if (json::jtype::peek(next) == json::jtype::not_valid)
      return REJECTED;
  ...
  case ARRAY_READING_VALUE:
    assert(this->sub_reader != NULL);
    if (this->sub_reader->is_valid() && std::isspace(next))
      return WHITESPACE;
...
std::string json::parsing::decode_string(const char *input) {
  ...
  if (*index != '"')
    throw json::parsing_error("Expecting opening quote");
  index++;
  bool escaped = false;
...
json::parsing::parse_results json::parsing::parse(const char *input) {
  ...
  // Iterate
  while (!EMPTY_STRING(input) &&
         stream.push(*index) != json::reader::REJECTED) {
    index++;
  }
...
std::vector<std::string> json::parsing::parse_array(const char *input) {
  ...
  while (!EMPTY_STRING(index)) {
    ...
    if (parse_results.type == json::jtype::jstring) {
      result.push_back(
          json::parsing::decode_string(parse_results.value.c_str()));
    } else {
...
json::jobject::entry::operator int() const {
  return this->get_number<int>(INT_FORMAT);
}
json::jobject::entry::operator unsigned int() const {
  return this->get_number<unsigned int>(UINT_FORMAT);
}
json::jobject::entry::operator long() const {
  return this->get_number<long>(LONG_FORMAT);
}
json::jobject::entry::operator unsigned long() const {
  return this->get_number<unsigned long>(ULONG_FORMAT);
}
json::jobject::entry::operator char() const {
  return this->get_number<char>(CHAR_FORMAT);
}
json::jobject::entry::operator float() const {
  return this->get_number<float>(FLOAT_FORMAT);
}
json::jobject::entry::operator double() const {
  return this->get_number<double>(DOUBLE_FORMAT);
}

json::jobject::entry::operator std::vector<int>() const {
  return this->get_number_array<int>(INT_FORMAT);
}
json::jobject::entry::operator std::vector<unsigned int>() const {
  return this->get_number_array<unsigned int>(UINT_FORMAT);
}
json::jobject::entry::operator std::vector<long>() const {
  return this->get_number_array<long>(LONG_FORMAT);
}
json::jobject::entry::operator std::vector<unsigned long>() const {
  return this->get_number_array<unsigned long>(ULONG_FORMAT);
}
json::jobject::entry::operator std::vector<char>() const {
  return this->get_number_array<char>(CHAR_FORMAT);
}
json::jobject::entry::operator std::vector<float>() const {
  return this->get_number_array<float>(FLOAT_FORMAT);
}
json::jobject::entry::operator std::vector<double>() const {
  return this->get_number_array<double>(DOUBLE_FORMAT);
}
...
void json::jobject::proxy::set_array(const std::vector<std::string> &values,
  ...
  for (size_t i = 0; i < values.size(); i++) {
    if (wrap)
      value += json::parsing::encode_string(values[i].c_str()) + ",";
    else
      value += values[i] + ",";
  }
  if (values.size() > 0)
...
json::jobject::operator std::string() const {
  if (is_array()) {
    ...
    std::string result = "{";
    for (size_t i = 0; i < this->size(); i++) {
      result += json::parsing::encode_string(this->data.at(i).first.c_str()) +
                ":" + this->data.at(i).second + ",";
    }
    result.erase(result.size() - 1, 1);
    result += "}";
    return result;
...
std::string json::jobject::pretty(unsigned int indent_level) const {
  ...
  if (is_array()) {
    ...
    for (size_t i = 0; i < this->size(); i++) {
      ...
      case json::jtype::jarray:
      case json::jtype::jobject:
        result += json::jobject::parse(this->data.at(i).second)
                      .pretty(indent_level + 1);
        break;
      default:
    ...
    for (size_t i = 0; i < this->size(); i++) {
      ...
      case json::jtype::jobject:
        result += std::string(
            json::parsing::tlws(json::jobject::parse(this->data.at(i).second)
                                    .pretty(indent_level + 1)
                                    .c_str()));
        break;
      default:
