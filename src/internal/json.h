 ...
 *
 * \note The function will only determine the type of the first value
...
public:
  enum push_result {
    ...
    WHITESPACE ///< The character was whitespace. Reading should continue but
               ///< the whtiespace was not stored.
  };
   ...
   * \returns `ACCEPTED` if the value was added to the reader, `WHITESPACE` if
   * the input was whitespace that was not stored, and `REJECTED` is the input
   * was invalid for the value type
   */
  virtual push_result push(const char next);

  /*!\brief Checks the value
   *
   * \returns The type of value stored in the reader, or `not_valid` if no value
   * is stored
   */
  inline virtual jtype::jtype type() const {
    return this->length() > 0 ? jtype::peek(this->front())
   ...
   * \warning This method will return the value regardless of the state of the
   * value, valid or not
   */
...
protected:
   ...
   *
   * This template is intended for use with #string_reader_enum,
   * #number_reader_enum, #array_reader_enum, and #object_reader_enum
   */
  template <typename T> T get_state() const {
   ...
   * #number_reader_enum, #array_reader_enum, and #object_reader_enum
   */
  template <typename T> void set_state(const T state) {
  ...
  enum string_reader_enum {
    ...
    STRING_OPENING_QUOTE, ///< The opening quote has been read. Equivalant to
                          ///< #STRING_OPEN, but used for debugging the state
    STRING_OPEN, ///< The opening quote has been read and the last character was
                 ///< not an escape character
    STRING_ESCAPED, ///< The last character was an reverse solidus (\),
                    ///< indicating the next character should be a control
                    ///< character
    STRING_CODE_POINT_START, ///< An encoded unicode character is encountered.
                             ///< Expecting four following hex digits.
    STRING_CODE_POINT_1,     ///< An encoded unicode character is encountered.
                             ///< Expecting three following hex digits (one has
                             ///< already been read).
    STRING_CODE_POINT_2,     ///< An encoded unicode character is encountered.
                             ///< Expecting two following hex digits (two have
                             ///< already been read).
    STRING_CODE_POINT_3,     ///< An encoded unicode character is encountered.
                             ///< Expecting one following hex digit (three has
                             ///< already been read).
  ...
  enum number_reader_enum {
    ...
    NUMBER_OPEN_NEGATIVE,   ///< A negative value has been read as the first
                            ///< character
    ...
    NUMBER_FRACTION_DIGITS, ///< A decimal point and subsequent digits were the
                            ///< last values read
    NUMBER_EXPONENT,        ///< An exponent indicator has been read
    NUMBER_EXPONENT_SIGN,   ///< An exponent sign has been read
    NUMBER_EXPONENT_DIGITS ///< An exponent indicator and subsequent digits were
  ...
  enum array_reader_enum {
    ...
    ARRAY_AWAITING_NEXT_LINE, ///< An array value has been read and a comma was
                              ///< encountered. Expecting new line.
  ...
  enum object_reader_enum {
    ...
    OBJECT_AWAITING_NEXT_LINE, ///< An object key value pair has been read and a
                               ///< comma was encountered. Expecting new line.
 ...
 *
 * \details Given a string, returns a pointer to the first character that is not
 * white space. Spaces, tabs, and carriage returns are all considered white
 * space.
 * @param start The string to examine
 * @return A pointer within the input string that points at the first charactor
 * that is not white space \note If the string consists of entirely white space,
 * then the null terminator is returned \warning The behavior of this function
 * with string that is not null-terminated is undefined
 */
const char *tlws(const char *start);

/*! \brief Reads a set of digits from a string
 *
 * \details This function will take an input string and read the digits at the
 * front of the string until a character other than a digit (0-9) is
 * encountered.
 ...
 *
 * \details The quotation mark ("), reverse solidus (\), solidus (/), backspace
 * (b), formfeed (f), linefeed (n), carriage return (r), horizontal tab (t), and
 * Unicode character will be unescaped
 * @param input A string, encapsulated in quotations ("), potentially containing
 ...
 * \details The quotation mark ("), reverse solidus (\), solidus (/), backspace
 * (b), formfeed (f), linefeed (n), carriage return (r), horizontal tab (t), and
 * Unicode character will be escaped
 * @param input A string potentially containing control characters
 * @return A string that has all control characters escaped with a reverse
 ...
 * \exception json::parsing_error Exception thrown when the input is not valid
 * JSON
 */
 ...
 *
 * \details Converts a serialized JSON array into a vector of the values in the
 * array
 * @param input The serialized JSON array
 * @return A vector containing each element of the array with each element being
 * serialized JSON
 */
...
public:
   ...
   *
   * @param array If true, the instance is initialized as an array. If false,
   * the instance is initalized as an object.
   */
  inline jobject(bool array = false) : array_flag(array) {}

  /*! \brief Copy constructor */
  inline jobject(const jobject &other)
      : data(other.data), array_flag(other.array_flag) {}
   ...
   *
   * @return True if the instance represents an array, false if the instance
   ...
   *
   * \todo Currently, the comparison just seralizes both objects and compares
   * the strings, which is probably not as efficent as it could be
   */
   ...
   *
   * \exception json::parsing_error Thrown if the key-value is incompatable with
   * the existing object (object/array mismatch)
   */
   ...
   * \exception json::parsing_error Thrown when the input string is not valid
   * JSON
   */
   ...
   */
  static inline jobject parse(const std::string input) {
   ...
   * @param[out] output Should the parsing attempt be successful, the resultant
   * JSON object or array
   * @return True of the parsing attempt was successful and false if the parsing
   ...
   * @return True if the object contains the provided key and false if the
   * object does not contain the key \note If the object represents a JSON
   * array, then this function will always return false
   */
   ...
   *
   * @return A list of keys contained in the object. If the object is actionally
   * an array, an empty list will be returned
   */
  key_list_t list_keys() const;

  /*! \brief Sets the value assocaited with the key
   *
   * \details If the key exists, then the value is updated. If the key does not
   * exist, then the key value pair is added to the object.
   * @param key The key for the entry
   * @param value The value for the entry
   * \exception json::invalid_key Exception thrown if the object actually
   ...
   * \exception json::invalid_key Exception thrown if the key does not exist in
   * the object or the object actually represents a JSON array
   */
  ...
  protected:
     ...
     * @param format The format used to convert the serialized value to a
     * numeric value
     ...
     * @param format The format used to convert the serialized values in the
     * array to numeric values
     * @return The value as a vector of numbers
     */
    template <typename T>
    inline std::vector<T> get_number_array(const char *format) const {
      std::vector<std::string> numbers =
          json::parsing::parse_array(this->ref().c_str());
      std::vector<T> result;
      for (size_t i = 0; i < numbers.size(); i++) {
        result.push_back(
            json::parsing::get_number<T>(numbers[i].c_str(), format));
      }
      return result;
    }
  ...
  public:
    ...
    operator std::vector<std::string>() const {
      return json::parsing::parse_array(this->ref().c_str());
    }
     ...
     */
    template <typename T> inline std::vector<T> as_array() const {
      return (std::vector<T>)(*this);
    }

    /*! \brief Returns true if the value is a string */
    inline bool is_string() const {
      return json::parsing::parse(this->ref().c_str()).type ==
             json::jtype::jstring;
    }
    ...
    inline bool is_true() const {
      json::parsing::parse_results result =
          json::parsing::parse(this->ref().c_str());
  ...
  public:
     ...
     *
     * This method assumed the entry contains a JSON object and returns another
     * constant value from within
     *
     ...
     *
     * This method assumed the entry contains a JSON array and returns another
     * constant value from within
     *
   ...
   *
   * This proxy is more memory-efficent than a json::jobject::const_value but
   * cannot use a JSON array as a source
   */
  ...
  protected:
     ...
     * @param wrap When true, the values are wrapped in quotes. When false, the
     * values are stored as-is
     */
    void set_array(const std::vector<std::string> &values,
     ...
     */
    template <typename T>
    inline void set_number_array(const std::vector<T> &values,
                                 const char *format) {
      std::vector<std::string> numbers;
  ...
  public:
    /*! \brief Constructor
     *
     * @param source The JSON object that will be updated when a value is
     * assigned
     * @param key The key for the value to be updated
     */
    proxy(jobject &source, const std::string key)
        : json::jobject::const_proxy(source, key), sink(source) {}
     ...
     *
     * This method is required because operator=(bool value) conflict with
     * number-based assignments
     */
   ...
   * \exception json::invalid_key Exception thrown if the object is actually a
   * JSON array
   */
   ...
   * JSON array
   */
  inline virtual const jobject::const_proxy
   ...
   *
   * \note While this method is intended for JSON arrays, this method is also
   * valid for JSON objects
   */
   ...
   * \note The serialized object or array will be in the most compact form and
   * will not contain any extra white space, even if the serialized string used
   * to generate the object or array contained extra white space.
   */
  inline std::string as_string() const { return this->operator std::string(); }

  /*! \brief Returns a pretty (multi-line indented) serialzed representation of
   * the object or array
   *
